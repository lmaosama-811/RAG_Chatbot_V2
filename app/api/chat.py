...
from ..schemas.request_model import ChatbotRequest, GovernmentChatRequest
...
@router.post("/upload_file",response_model=ChatBotResponse)
@limiter.limit("10/minute") #10 request each minute each IP
def chat_upload_file(request: Request,query: ChatbotRequest, db: Session = Depends(get_session)):
...
@router.post("/government_data",response_model=ChatBotResponse)
@limiter.limit("10/minute")
async def chat_government_data(request: Request,query: GovernmentChatRequest, db: Session = Depends(get_session)):
    """Ask question about government data"""
    logger.info("Chat request received")

    #Dynamic government data session
    session_id = (CM_service.generate_session_id() if query.session_id is None else query.session_id)
    ...
    if q_type == "chit_chat":
        ...
        answer = llm_service.ask_model(llm, "direct_answer", user_content)
        try: 
            db_service.create_dialog(session_id,session_name,"user",user_content,db)
            db_service.create_dialog(session_id,session_name,"assistant",answer.content,db)
        except Exception as e:
            db.rollback()
            logger.error(f"{e}")
            raise
        return ChatBotResponse(model_name=llm.model_name,session_id=session_id,session_name=session_name,answer=answer.content,confidence=1.0)
    ...
    try: 
        db_service.create_dialog(session_id,session_name,"user",user_content,db)
        db_service.create_dialog(session_id,session_name,"assistant",model_output.content,db)
    except Exception as e:
        db.rollback()
        logger.error(f"{e}")
        raise
    def event_generator():
        try:
            ...
            if full_response:
                try: 
                    db_service.create_dialog(session_id,session_name,"user",user_content,db)
                    db_service.create_dialog(session_id,session_name,"assistant",full_response,db)
                except Exception as e:
                    db.rollback()
                    logger.error(f"{e}")
                    raise
    ...
    logger.info(f"Check hallucination successfully. Confidence: {confidence}")
    # return StreamingResponse(event_generator(),media_type="text/plain") #Use this if we want streaming response
    return ChatBotResponse(model_name=llm.model_name,session_id=session_id,session_name=session_name,answer=model_output.content + disclaimer, confidence=confidence)

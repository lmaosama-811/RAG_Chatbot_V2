...
class DBService:
    ...
    def get_file_status(self,file_id,db):
        ...
        return db.exec(cmd).all() #List[Tuple]
    def get_file_type(self,file_id,db):
        cmd = select(FileStatus.type).where(FileStatus.file_id==file_id)
        return db.exec(cmd).first()
    def create_file(self,db,file_id,type,status="PENDING"):
        new_file = FileStatus(file_id=file_id,type=type,status=status)
        db.add(new_file)
        db.commit()
    ...
    def get_parent_chunk(self,parent_id,db):
        ...
        return db.exec(cmd).all() #ParentStore
